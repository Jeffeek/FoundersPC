...
namespace FoundersPC.Services.Hardware_Services.Hardware.Memory
{
	public class SSDService : ISSDService
	{
		...
		public async Task<IEnumerable<SSDReadDto>> GetAllSSDsAsync() =>
			_mapper.Map<IEnumerable<SSD>, IEnumerable<SSDReadDto>>(await _unitOfWork.SSDsRepository
				                                                       .GetAllAsync());

		/// <inheritdoc />
		public async Task<SSDReadDto> GetSSDByIdAsync(int ssdId) =>
			_mapper.Map<SSD, SSDReadDto>(await _unitOfWork.SSDsRepository.GetByIdAsync(ssdId));
