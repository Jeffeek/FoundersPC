...
(function() {
    if (!window.performance.now) {
        window.performance.now =
            (!Date.now) ? function() { return new Date().getTime(); } : function() { return Date.now(); };
    }
})();
...
var PLAYER_CLIP_RECT = { x: 0, y: 204, w: 100, h: 32 };
...
var ALIEN_X_MARGIN = 55;
var ALIEN_SQUAD_WIDTH = 10 * ALIEN_X_MARGIN;
...
var BaseSprite = Class.extend({
    ...
    _updateBounds: function() {
        this.bounds.set(this.position.x,
            this.position.y,
...
function setupAlienFormation() {
    alienCount = 0;
    const len = window.innerWidth / 25;
    for (let i = 0; i < len; i++) {
        const gridX = (i % 25);
