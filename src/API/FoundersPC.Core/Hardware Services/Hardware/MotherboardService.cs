...
namespace FoundersPC.Services.Hardware_Services.Hardware
{
	public class MotherboardService : IMotherboardService
	{
		...
		/// <inheritdoc />
		public async Task<IEnumerable<MotherboardReadDto>> GetAllMotherboardsAsync() => _mapper.Map<IEnumerable<Motherboard>, IEnumerable<MotherboardReadDto>>(await _unitOfWork
																																									 .MotherboardsRepository
																																									 .GetAllAsync());

		/// <inheritdoc />
		public async Task<MotherboardReadDto> GetMotherboardByIdAsync(int motherboardId) => _mapper.Map<Motherboard, MotherboardReadDto>(await _unitOfWork
																																			   .MotherboardsRepository
		...
		public async Task<bool> CreateMotherboard(MotherboardInsertDto motherboard)
		{
			...
			if (await _unitOfWork.MotherboardsRepository.AnyAsync(mappedMotherboard)) return false;
