...
namespace FoundersPC.Services.Hardware_Services.Hardware.Memory
{
	public class RAMService : IRAMService
	{
		...
		public async Task<IEnumerable<RAMReadDto>> GetAllRAMsAsync() =>
			_mapper.Map<IEnumerable<RAM>, IEnumerable<RAMReadDto>>(await _unitOfWork.RAMsRepository
				                                                       .GetAllAsync());

		/// <inheritdoc />
		public async Task<RAMReadDto> GetRAMByIdAsync(int ramId) =>
			_mapper.Map<RAM, RAMReadDto>(await _unitOfWork.RAMsRepository.GetByIdAsync(ramId));
