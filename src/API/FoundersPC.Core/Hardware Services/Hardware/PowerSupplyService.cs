...
namespace FoundersPC.Services.Hardware_Services.Hardware
{
	public class PowerSupplyService : IPowerSupplyService
	{
		...
		/// <inheritdoc />
		public async Task<IEnumerable<PowerSupplyReadDto>> GetAllPowerSuppliersAsync() => _mapper.Map<IEnumerable<PowerSupply>, IEnumerable<PowerSupplyReadDto>>(await _unitOfWork
																																									   .PowerSuppliersRepository
																																									   .GetAllAsync());

		/// <inheritdoc />
		public async Task<PowerSupplyReadDto> GetPowerSupplyByIdAsync(int powerSupplyId) => _mapper.Map<PowerSupply, PowerSupplyReadDto>(await _unitOfWork
																																			   .PowerSuppliersRepository
		...
		public async Task<bool> CreatePowerSupply(PowerSupplyInsertDto powerSupply)
		{
			...
			if (await _unitOfWork.PowerSuppliersRepository.AnyAsync(mappedPowerSupply)) return false;
