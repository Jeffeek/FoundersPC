...
namespace FoundersPC.API.Services.Hardware_Services.Hardware.Memory
{
    public class RandomAccessMemoryService : IRandomAccessMemoryService
    {
        ...
        {
            var items = _mapper.Map<IEnumerable<RandomAccessMemory>, IEnumerable<RandomAccessMemoryReadDto>>(await _unitOfWorkHardwareAPI
        ...
        public async Task<IEnumerable<RandomAccessMemoryReadDto>> GetAllRandomAccessMemoryAsync() =>
            _mapper.Map<IEnumerable<RandomAccessMemory>, IEnumerable<RandomAccessMemoryReadDto>>(await _unitOfWorkHardwareAPI
                                                                                                           .RandomAccessMemoryRepository
                                                                                                           .GetAllAsync());

        /// <inheritdoc/>
        public async Task<RandomAccessMemoryReadDto> GetRandomAccessMemoryByIdAsync(int ramId) =>
            _mapper.Map<RandomAccessMemory, RandomAccessMemoryReadDto>(await _unitOfWorkHardwareAPI.RandomAccessMemoryRepository.GetByIdAsync(ramId));

        /// <inheritdoc/>
        public async Task<bool> CreateRandomAccessMemoryAsync(RandomAccessMemoryInsertDto randomAccessMemory)
        {
            var mappedRAM = _mapper.Map<RandomAccessMemoryInsertDto, RandomAccessMemory>(randomAccessMemory);
