...
public class TokenRequestHandler : IRequestHandler<TokenRequest, TokenResponse>
{
    ...
    public TokenRequestHandler(UserManager<ApplicationUser> userManager,
    ...
    private async Task<TokenResponse> RefreshTokenAsync(TokenRequest request)
    {
        ...
        try
        {
            ...
            if (user.IsActive)
                throw new BadRequestException("invalid_grant", "User is inactive");
    ...
    private TokenResponse GenerateToken(ClaimsPrincipal principal, ApplicationUser user)
    {
        ...
        var expires = now.Add(TimeSpan.FromHours(_authOptions.MinutesToExpire));
